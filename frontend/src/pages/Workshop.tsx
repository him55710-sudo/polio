          ...
          <div className="mt-3 space-y-2.5">
            {message.topicSuggestions.map((topic, index) => (
              <button
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-left transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">주제 {index + 1}</p>
                <p className="mt-1 text-sm font-bold text-slate-900">{topic.title}</p>
                <p className="mt-1 text-xs font-medium text-slate-600">
                  {selectingTopicId === topic.id ? '선택한 주제로 초안을 생성하는 중입니다...' : '클릭하면 오른쪽 초안 패널을 바로 채웁니다.'}
                </p>
              </button>
            ))}
          </div>
...
export function Workshop() {
  ...
  const handleGuidedTopicSelection = useCallback(
    async (topicId: string, subjectFromMessage?: string) => {
      ...
      try {
        ...
        toast.success('선택한 주제 기준으로 오른쪽 초안 패널을 채웠습니다.');
      } catch (error) {
        console.error('Guided topic selection failed:', error);
        toast.error('주제 선택 결과를 반영하지 못했습니다.');
      } finally {
        setIsSelectingGuidedTopicId(null);
      }
    },
  ...
  const inputPlaceholder = isProjectBacked && !isGuidedTopicSelected
    ? '예: 수학, 생명과학, 사회문화'
    : '논지 구성, 근거 연결, 문장 다듬기 등 요청을 입력해 주세요.';
        ...
        >
          ...
          {isProjectBacked && !isGuidedTopicSelected ? (
            <WorkflowNotice
              tone="info"
              title="주제 먼저 선택해 주세요"
              description="과목/주제를 입력하면 Foli가 3개 주제를 제안하고, 선택 즉시 오른쪽 초안 패널을 채웁니다."
            />
          ) : null}
          ...
          <div className="sticky bottom-0 z-20 -mx-3 flex items-center gap-2 border-t border-slate-200 bg-white/95 px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-10px_22px_rgba(15,23,42,0.08)] backdrop-blur md:static md:mx-0 md:border-0 md:bg-transparent md:px-0 md:pt-0 md:pb-0 md:shadow-none">
            ...
            <input
              ...
              disabled={Boolean(isSelectingGuidedTopicId)}
            ...
            <PrimaryButton
              ...
              disabled={!input.trim() || isTyping || Boolean(isSelectingGuidedTopicId)}
              aria-label="메시지 전송"
              size="icon"
            >
              <Send size={16} />
