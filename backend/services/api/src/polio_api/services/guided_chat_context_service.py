...
from polio_api.services.guided_chat_state_service import GUIDED_CHAT_STATE_DRAFT_TITLE, load_guided_chat_state
...
def _derive_evidence_gaps(
    *,
    ...
    if invalid_project_requested:
        gaps.append("요청하신 프로젝트를 찾지 못해 최근 프로젝트 기준으로 안내드립니다.")
    if project is None:
        gaps.append("프로젝트 맥락이 없어 사용자 기본 정보만 기반으로 안내드립니다.")
    if not context.known_target_info.get("target_university") and not context.known_target_info.get("target_major"):
        gaps.append("목표 대학/학과 정보가 확인되지 않았습니다.")
    if context.diagnosis_summary is None:
        gaps.append("진단 결과가 없어 약식 제안으로 안내드립니다.")
    ...
    if not context.workshop_history and not context.prior_draft_signals:
        gaps.append("기존 작성 흐름 신호가 적어 보수적으로 제안드립니다.")
