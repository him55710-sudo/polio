...
_DEFAULT_TEMPLATE_BY_MODE: dict[str, str] = {
    ...
    "premium": "consultant_diagnosis_premium_10p",
...
def _build_student_score_block(
    *,
    ...
    if not missing_evidence:
        adjusted_score, clamp_reason = _calibrate_student_dashboard_score(
            key=key,
    ...
    if missing_evidence:
        ...
        uncertainty_note = f"보수적 제한: {clamp_reason}"
        missing_evidence.append(clamp_reason)
    else:
...
def _calibrate_student_dashboard_score(
    *,
    ...
    if calibrated >= 90 and not exceptional_ready:
        calibrated = 89
        clamp_reason = f"우수(90+) 등급을 진단하기에는 정량적 증거 요건(앵커 {required_anchor_count + 3}개 이상, 페이지 다양성 {required_page_diversity + 2}개 이상, 커버리지 {round(coverage_score * 100)}% / 기준 86% 이상)이 부족하여 89점으로 제한되었습니다."
    elif calibrated >= 80 and not strong_ready:
        calibrated = 79
        clamp_reason = f"양호(80+) 등급을 진단하기에는 정량적 증거 요건(앵커 {required_anchor_count + 1}개 이상, 페이지 다양성 {required_page_diversity}개 이상, 커버리지 {round(coverage_score * 100)}% / 기준 72% 이상)이 부족하여 79점으로 제한되었습니다."
