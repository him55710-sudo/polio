    ...
    *,
    ...
    def _merge(
        ...
        if semantic_grade is None:
            score, clamp_reason = _calibrate_positive_axis_score(key=key, score=base, features=features)
            hints = []
            if clamp_reason:
                hints.append(clamp_reason)
            return score, "", hints
        score = _bounded_int(base * 0.3 + semantic_grade.score * 0.7)
        score, clamp_reason = _calibrate_positive_axis_score(key=key, score=score, features=features)
        hints = list(semantic_grade.evidence_hints) if semantic_grade.evidence_hints else []
        if clamp_reason:
            hints.append(clamp_reason)
        return score, semantic_grade.rationale, hints
...
def _calibrate_positive_axis_score(
    *,
    ...
    if calibrated >= 90 and not _positive_axis_allows_90(key=key, features=features, active_sections=active_sections):
        calibrated = 89
        clamp_reason = "우수(90+) 증명 보완 필요: 양적 검증 요건(신뢰도 0.85 이상, 세특 구체성 0.55 이상 등) 미달로 보수적 점수 클램프(89점) 적용"
    elif calibrated >= 80 and not _positive_axis_allows_80(key=key, features=features, active_sections=active_sections):
        calibrated = 79
        clamp_reason = "양호(80+) 증명 보완 필요: 양적 검증 요건(신뢰도 0.68 이상, 세특 구체성 0.30 이상 등) 미달로 보수적 점수 클램프(79점) 적용"
    return calibrated, clamp_reason
