...
_TOKEN_RE = re.compile(r"[A-Za-zê°€-??{2,}")
...
def extract_student_record_features(
    *,
    ...
    for document in documents:
        ...
        if structured_data:
            ...
            if "canonical_data" in structured_data:
                ...
                if attendance:
                    section_presence["êµê³¼?™ìŠµë°œë‹¬?í™©"] = True # Attendance usually in the same section or nearby
                    section_record_counts["êµê³¼?™ìŠµë°œë‹¬?í™©"] += len(attendance)

                # Awards
                awards = _ensure_list(canonical.get("awards"))
                if awards:
                    section_presence["?˜ìƒê²½ë ¥"] = True
                    section_record_counts["?˜ìƒê²½ë ¥"] += len(awards)
                    total_records += len(awards)

                # Grades
                grades = _ensure_list(canonical.get("grades"))
                if grades:
                    section_presence["êµê³¼?™ìŠµë°œë‹¬?í™©"] = True
                    section_record_counts["êµê³¼?™ìŠµë°œë‹¬?í™©"] += len(grades)
                ...
                if extra:
                    section_presence["ì°½ì˜?ì²´?˜í™œ??] = True
                    section_record_counts["ì°½ì˜?ì²´?˜í™œ??] += len(extra)
                ...
                if subj_notes:
                    section_presence["êµê³¼?™ìŠµë°œë‹¬?í™©"] = True
                    section_record_counts["êµê³¼?™ìŠµë°œë‹¬?í™©"] += len(subj_notes)
                ...
                if reading:
                    section_presence["?…ì„œ?œë™"] = True
                    section_record_counts["?…ì„œ?œë™"] += len(reading)
                    for r in reading:
                ...
                if behavior:
                    section_presence["?‰ë™?¹ì„± ë°?ì¢…í•©?˜ê²¬"] = True
                    section_record_counts["?‰ë™?¹ì„± ë°?ì¢…í•©?˜ê²¬"] += 1
    ...
    if needs_review_documents > 0:
        risk_flags.append("?¼ë? ë¬¸ì„œê°€ needs_review ?íƒœ?…ë‹ˆ??")
    if evidence_density < 0.35:
        risk_flags.append("ê·¼ê±° ë°€?„ê? ??•„ ì£¼ì¥ ê²€ì¦??¬ìœ ê°€ ë¶€ì¡±í•©?ˆë‹¤.")
    if overlap_ratio < 0.25 and major_terms:
        risk_flags.append("ëª©í‘œ ?„ê³µ ê´€???¤ì›Œ?œê? ê¸°ë¡ ?„ë°˜??ì¶©ë¶„???œëŸ¬?˜ì? ?ŠìŠµ?ˆë‹¤.")
