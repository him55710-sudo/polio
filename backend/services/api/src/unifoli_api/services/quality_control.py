...
def resolve_advanced_features(
    *,
    ...
    profile = get_quality_profile(quality_level)
    if not requested:
        return False, "고급 ??장?? ??청???? ??았??니??"
    if not profile.advanced_features_allowed:
        return False, f"{profile.label}??서??고급 ??장????용???? ??습??다."
    if reference_count < profile.minimum_reference_count:
        return (
            False,
            f"{profile.label}에서는 참고자료 {profile.minimum_reference_count}개 이상일 때만 고급 확장 기능을 허용합니다.",
        )
    return True, f"{profile.label} 기준에서 참고자료 조건을 충족해 고급 확장 기능을 허용합니다."
