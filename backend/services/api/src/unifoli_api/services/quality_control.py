...
from unifoli_domain.enums import QualityLevel, TurnType
...
def build_render_requirements(
    *,
    ...
    if context_score < profile.render_threshold:
        missing.append(f"ë§¥ë½ ?ìˆ˜ {profile.render_threshold - context_score}??ë¶€ì¡?)
    if turn_count < profile.minimum_turn_count:
        missing.append(f"?€????{profile.minimum_turn_count - turn_count}ê°?ë¶€ì¡?)
    if reference_count < profile.minimum_reference_count:
        missing.append(f"ì°¸ê³ ?ë£Œ {profile.minimum_reference_count - reference_count}ê°?ë¶€ì¡?)
...
def resolve_advanced_features(
    *,
    ...
    profile = get_quality_profile(quality_level)
    if not requested:
        return False, "ê³ ê¸‰ ?•ì¥?€ ?”ì²­?˜ì? ?Šì•˜?µë‹ˆ??"
    if not profile.advanced_features_allowed:
        return False, f"{profile.label}?ì„œ??ê³ ê¸‰ ?•ì¥???ˆìš©?˜ì? ?ŠìŠµ?ˆë‹¤."
    if reference_count < profile.minimum_reference_count:
        return (
            False,
            f"{profile.label}?ì„œ??ì°¸ê³ ?ë£Œ {profile.minimum_reference_count}ê°??´ìƒ???ˆì„ ?Œë§Œ ê³ ê¸‰ ?•ì¥???ìš©?©ë‹ˆ??",
        )
    return True, f"{profile.label} ê¸°ì?ê³?ì°¸ê³ ?ë£Œ ì¡°ê±´??ì¶©ì¡±?˜ì–´ ê³ ê¸‰ ?•ì¥???ìš©?©ë‹ˆ??"
...
def build_starter_choices(
    *,
    ...
    profile = get_quality_profile(quality_level)
    quest_label = quest_title or "?´ë²ˆ ?êµ¬"
    major_label = target_major or "?¬ë§ ?„ê³µ"
    output_label = (recommended_output_type or "?êµ¬ ê²°ê³¼ë¬?).lower()
