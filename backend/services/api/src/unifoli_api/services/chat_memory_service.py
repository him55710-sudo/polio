...
def _collect_confirmed_evidence_points(session: WorkshopSession) -> list[str]:
    ...
    for turn in reversed(session.turns or []):
        ...
        if not query:
            continue
        if any(token in query for token in ("근거", "기록", "문서", "출처")):
...
def build_workshop_memory_payload(
    ...
    *,
    ...
    if quest and getattr(quest, "title", None):
        goal_lines.append(f"목표 제출물: {quest.title}")
    blocks.append("[워크숍 목표]\n" + "\n".join(goal_lines))

    blocks.append("[가이드 상태]\n" + json.dumps(summary, ensure_ascii=False, indent=2))
    ...
    if older_count:
        blocks.append(f"[이전 대화 요약]\n이전 대화 {older_count}건은 생략하고 최근 흐름만 유지합니다.")
