...
def _build_topic_prompt(*, subject: str, context: GuidedChatContext, starred_keywords: list[str]) -> str:
    ...
    return (
        ...
        f"{reference_text}\n\n"
        "[추천 주제 생성 지침]\n"
        "위 '우수 탐구 사례'들의 수준과 설득력을 참고하여, 학생의 개별 맥락에 최적화된 독창적인 탐구 주제 3개를 제안하세요.\n"
...
def _build_fallback_topics(
    *,
    ...
    used_types = {item.suggestion_type for item in existing if item.suggestion_type}
    ...
    search_service = get_topic_search_service()
    target_major = context.known_target_info.get("target_major", "")
    query = f"{subject} {target_major}"
    search_results = search_service.search(query, limit=10)
    ...
    for item in search_results:
        candidates.append((
            item["label"],
            item["reason"],
            "주제 라이브러리의 검증된 탐구 모델을 적용했습니다.",
            item.get("suggestion_type", "subject")
        ))

    result: list[TopicSuggestion] = []
    next_index = len(existing) + 1
    for title, fit, novelty, suggestion_type in candidates:
        if title in existing_titles or suggestion_type in used_types:
            continue
        result.append(
            TopicSuggestion(
                id=f"fallback-{suggestion_type}",
                title=title,
                why_fit_student=fit,
