...
def _build_dimension(
    *,
    ...
    return SafetyDimension(
        ...
        label=label,
        score=clamped,
        status=_dimension_status(clamped),
        detail=detail,
...
def run_safety_check(
    ...
    level_detail = (
        "학생 수준에 맞는 표현 범위를 유지하고 있습니다."
        if level_score >= 80
        else (
            f"심화 표현 {len(advanced_hits)}건이 감지되었고 "
            f"학생 맥락으로 확인되지 않은 표현이 {len(unsupported_advanced_hits)}건 있습니다."
        )
    )
    ...
    feasibility_detail = (
        ...
        else f"실행 강도가 높은 표현 {len(feasibility_hits)}건이 맥락 대비 과하게 보입니다."
    )
    ...
    fabrication_detail = (
        "허위 경험이나 과장된 수치 징후가 크지 않습니다."
        if fabrication_score >= 80
        else (
            f"근거가 약한 경험 서술 {len(unsupported_experience_hits)}건과 "
    ...
    ai_detail = (
        "학생 말투에 가까운 표현을 유지하고 있습니다."
        if ai_score >= 80
        else f"범용적인 AI 문체 신호가 {len(ai_hits)}건 감지되었습니다."
    )
    ...
    if reference_mentions and reference_count == 0:
        reference_score -= 20
    reference_detail = (
        "참고자료 활용 강도가 현재 품질 수준에 맞습니다."
        if reference_score >= 80
        else (
            f"현재 품질에서는 최소 {profile.minimum_reference_count}개의 참고자료가 권장되며 "
            "출처 표현 대비 실제 참고자료 수가 부족합니다."
        )
    )
    ...
    expansion_ratio = output_char_count / max(input_char_count, 100)
    ghostwriting_score = 100
    if expansion_ratio > 8 and output_char_count > 400:
        ghostwriting_score -= round((expansion_ratio - 8) * 12)
    ghostwriting_detail = (
        "학생 맥락 기반의 적절한 분량으로 보입니다."
        if ghostwriting_score >= 80
        else (
            f"학생 입력({input_char_count}자)에 비해 생성 결과({output_char_count}자)가 지나치게 길어 "
            "대필 위험이 높아 보입니다."
        )
    )
    ...
    if not flags:
        summary = (
            f"학생 맥락에 맞는 {recommended_profile.label} 수준의 결과이며 "
            "허위 경험이나 과장 위험이 크게 보이지 않습니다."
        )
    else:
        summary = (
            f"안전 점검에서 {len(flags)}개의 주의 신호가 감지되었고 "
            f"최종 권장 품질 수준은 {recommended_profile.label}입니다."
        )
