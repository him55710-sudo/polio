...
_SECTION_KEYWORDS: dict[str, tuple[str, ...]] = {
    "student_info": ("student", "name", "school", "id", "profile", "?¸ì ?¬í•­", "?™ì ?¬í•­", "?™ìƒëª?),
    "attendance": ("attendance", "absence", "late", "tardy", "present", "ì¶œê²°", "ê²°ì„", "ì§€ê°?, "ì¡°í‡´"),
    "awards": ("award", "prize", "competition", "?˜ìƒ", "?ìž¥", "?˜ìƒëª?),
    "creative_activities": ("creative", "club", "activity", "project", "ì°½ì˜??, "?™ì•„ë¦?, "?ìœ¨?œë™", "ì§„ë¡œ?œë™"),
    "volunteer": ("volunteer", "service", "hours", "ë´‰ì‚¬", "ë´‰ì‚¬?œê°„"),
    "grades_subjects": ("grade", "subject", "score", "evaluation", "êµê³¼", "?±ì ", "ë°œë‹¬?í™©"),
    "subject_special_notes": ("special note", "subject note", "comment", "?¸ë??¥ë ¥", "?¹ê¸°?¬í•­", "?¸íŠ¹"),
    "reading": ("reading", "book", "library", "?…ì„œ", "?„ì„œëª?),
    "behavior_general_comments": ("behavior", "general comment", "attitude", "?‰ë™?¹ì„±", "ì¢…í•©?˜ê²¬"),
}
...
def _build_pdf_summary(parsed: ParsedDocumentPayload, page_texts: list[str]) -> str:
    if not page_texts:
        return "PDF ?ìŠ¤?¸ë? ì¶©ë¶„??ì¶”ì¶œ?˜ì? ëª»í•´ ë¬¸ì„œ ?”ì•½ ê·¼ê±°ê°€ ?œí•œ?ìž…?ˆë‹¤."

    first = _clip(_normalize_sentence(page_texts[0]), 180)
    last = _clip(_normalize_sentence(page_texts[-1]), 180) if len(page_texts) > 1 else ""
    page_note = f"{parsed.page_count}?˜ì´ì§€ ë¬¸ì„œ?ì„œ ?µì‹¬ ?ë¦„???•ë¦¬?ˆìŠµ?ˆë‹¤."

    if last and last != first:
        return f"{page_note} ì²??˜ì´ì§€ ?”ì•½?€ {first} ë§ˆì?ë§??˜ì´ì§€ ?”ì•½?€ {last}"
    return f"{page_note} ?”ì•½: {first}"
...
def _build_evidence_gaps(parsed: ParsedDocumentPayload, page_texts: list[str]) -> list[str]:
    gaps: list[str] = []
    if not page_texts:
        gaps.append("?ìŠ¤?¸ë? ì¶©ë¶„??ì¶”ì¶œ?˜ì? ëª»í•´ ?˜ì´ì§€ë³?ê·¼ê±° ?•ì¸???„ìš”?©ë‹ˆ??")
    if parsed.needs_review:
        gaps.append("ë¬¸ì„œ ?ˆì§ˆ?´ë‚˜ ?¤ìº” ?íƒœë¥??¤ì‹œ ?•ì¸??ì¶”ì¶œ ?„ë½???†ëŠ”ì§€ ?ê???ì£¼ì„¸??")
    if parsed.page_count > len(page_texts):
        gaps.append("?¼ë? ?˜ì´ì§€?ì„œ ì¶”ì¶œ ?ìŠ¤?¸ê? ë¹„ì–´ ?ˆì–´ PDF ?ë¬¸ ?•ì¸???„ìš”?©ë‹ˆ??")
    warnings = parsed.warnings if isinstance(parsed.warnings, list) else []
    if warnings:
        gaps.append("ì¶”ê? ê²½ê³ ê°€ ?ˆì–´ ?œê¸°??ë¬¸ë‹¨ êµ¬ì¡°ê°€ ê¹¨ì?ì§€ ?Šì•˜?”ì? ?•ì¸???„ìš”?©ë‹ˆ??")
    if not gaps:
        gaps.append("?™ìƒë¶€ ì£¼ìš” ?¹ì…˜ë³?ê·¼ê±°ê°€ ì¶©ë¶„?œì? ìµœì¢… ê²€? ê? ?„ìš”?©ë‹ˆ??")
...
def _build_uncertainties(
    ...
    if not items:
        items.append("?„ìž¬ ?ë™ ë¶„ì„ ê²°ê³¼ë§Œìœ¼ë¡œëŠ” ?¸ë? ?´ì„???œê³„ê°€ ?ˆì–´ ?ë¬¸ ê²€? ê? ?„ìš”?©ë‹ˆ??")
...
def _extract_grades_subjects(text: str, pipeline_canonical: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    subjects = re.findall(r"(êµ?–´|?˜í•™|?ì–´|?¬íšŒ|??‚¬|ê³¼í•™|ë¬¼ë¦¬|?”í•™|?ëª…ê³¼í•™|ì§€êµ¬ê³¼???•ë³´|ë¯¸ìˆ |?Œì•…|ì²´ìœ¡)", text)
...
def _extract_student_profile(text: str, pipeline_canonical: dict[str, Any]) -> dict[str, Any]:
    ...
    name_match = re.search(r"(?:?™ìƒëª??±ëª…)\s*[:ï¼??\s*([ê°€-??{2,5})", text)
    if name_match and "student_name" not in profile:
        profile["student_name"] = name_match.group(1)

    school_match = re.search(r"([ê°€-?£A-Za-z0-9 ]+ê³ ë“±?™êµ)", text)
