...
from unifoli_domain.enums import EvidenceProvenance
...
_PLAIN_EQUATION_RE = re.compile(r"(?P<equation>[A-Za-zê°€-??[A-Za-z0-9ê°€-?? ()/\-*+^]{0,50}=\s*[A-Za-z0-9ê°€-?? ()/\-*+^.%]{1,60})")
_WORD_RE = re.compile(r"[A-Za-zê°€-??{2,}")
_NUMERIC_TOKEN_RE = re.compile(r"\d+(?:\.\d+)?(?:%|ëª?ê±???kg|g|cm|mm|?????œê°„|???")
...
def _build_chart_candidate(
    *,
    ...
    combined_text = f"{section['title']}\n{section['text']}".lower()
    chart_type = "line" if any(keyword in combined_text for keyword in {"trend", "trends", "ì¶”ì´"}) else "bar"
...
def _extract_numeric_datapoints(lines: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for raw_line in lines:
        ...
        if not cleaned:
            continue
        match = re.match(
            r"(?P<label>[^0-9]{2,40}?)[\s:ï¼?]+(?P<value>\d+(?:\.\d+)?)\s*(?P<unit>%|ëª?ê±???kg|g|cm|mm|?????œê°„|???",
            cleaned,
        )
        if not match:
            continue
...
def _extract_refs_from_value(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if key_text in {"source", "source_id", "supported_by", "citation", "reference", "ì¶œì²˜"}:
