...
_PLAIN_EQUATION_RE = re.compile(
    r"(?P<equation>[A-Za-z가-힣][A-Za-z0-9가-힣 ()/\-*+^]{0,50}=\s*[A-Za-z0-9가-힣 ()/\-*+^.%]{1,60})"
)
_WORD_RE = re.compile(r"[A-Za-z가-힣]{2,}")
_NUMERIC_TOKEN_RE = re.compile(r"\d+(?:\.\d+)?(?:%|kg|g|cm|mm|시간|개월|명)?")
...
def _extract_numeric_datapoints(lines: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for raw_line in lines:
        ...
        if not cleaned:
            continue
        match = re.match(
            r"(?P<label>[^0-9]{2,40}?)[\s:]+(?P<value>\d+(?:\.\d+)?)\s*(?P<unit>%|kg|g|cm|mm|시간|명)?",
            cleaned,
        )
        if not match:
            continue
