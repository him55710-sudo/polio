    ...
    *,
    ...
    try:
        ...
        _update_run_status(
            ...
            run,
            status="RUNNING",
            status_message="?ÖÎ°ú?úÎêú Î¨∏ÏÑú Í∑ºÍ±∞Î•?Í≤Ä?†ÌïòÍ≥??àÏäµ?àÎã§...",
        )
        ...
        _update_run_status(
            db,
            run,
            status_message="?ùÌôúÍ∏∞Î°ùÎ∂Ä ?πÏßï?êÏùÑ Ï∂îÏ∂ú?òÍ≥† ?àÏäµ?àÎã§...",
        )
        ...
        if should_use_llm and semantic_input_text:
            ...
            _update_run_status(
                db,
                run,
                status_message="Ï∂îÏ∂ú??Í∑ºÍ±∞Î•?Î∞îÌÉï?ºÎ°ú ?¨Ìôî ?òÎ? Î∂ÑÏÑù???òÌñâ?òÍ≥† ?àÏäµ?àÎã§...",
            )
            try:
                semantic_data = await asyncio.wait_for(
                    extract_semantic_diagnosis(
                        masked_text=semantic_input_text,
                        target_major=target_major or "?ºÎ∞ò ?ÑÌòï",
        ...
        if should_use_llm:
            _update_run_status(
                db,
                run,
                status_message="?ÅÏÑ∏ ÏßÑÎã® ?¥Ïö©???ùÏÑ±?òÍ≥† Í∑ºÍ±∞ ?∞Ïù¥?∞Î? Îß§Ìïë?òÍ≥† ?àÏäµ?àÎã§...",
            )
            try:
        ...
        run.status_message = "ÏßÑÎã®???ÑÎ£å?òÏóà?µÎãà??"
        ...
        try:
            ...
            latest_run.status_message = "ÏßÑÎã® ?§Ìñâ???§Ìå®?àÏäµ?àÎã§."
