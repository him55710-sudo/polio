    ...
    async def generate_json(
        ...
        if not content:
            raise LLMRequestError(
                "LLM ?‘ë‹µ??ë¹„ì–´ ?ˆì–´ ?œí•œ ëª¨ë“œë¡??„í™˜?©ë‹ˆ??",
        ...
        try:
            ...
            raise LLMRequestError(
                "?‘ë‹µ ?•ì‹???´ì„?˜ì? ëª»í•´ ?œí•œ ëª¨ë“œë¡??„í™˜?©ë‹ˆ??",
...
def _to_ollama_request_error(
    ...
    *,
    ...
    if reason in {"timeout", "unreachable"}:
        message = "AI ?‘ë‹µ??ì§€?°ë˜???œí•œ ëª¨ë“œë¡??„í™˜?©ë‹ˆ??"
    elif reason == "invalid_request":
        message = "AI ?”ì²­ êµ¬ì„±??ë§žì? ?Šì•„ ?œí•œ ëª¨ë“œë¡??„í™˜?©ë‹ˆ??"
    else:
        message = "AI ?¸ì¶œ ì¤??¤ë¥˜ê°€ ë°œìƒ???œí•œ ëª¨ë“œë¡??„í™˜?©ë‹ˆ??"
