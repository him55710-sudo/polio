...
def _build_state_response(
    *,
    ...
    default_message = (
        f"[{profile.label}] ?¥Îñ§ Î∞©Ïãù?ºÎ°ú ?úÏûë?†Ï? Í≥†Î•¥Î©?Í∑??òÏ???ÎßûÎäî ?®Ï†Ñ???åÌÅ¨???êÎ¶Ñ?ºÎ°ú ?¥Ïñ¥ÏßëÎãà??"
        if session.status == WorkshopStatus.COLLECTING_CONTEXT.value
        else f"[{profile.label}] Ï¥àÏïà Íµ¨ÏÑ±??Í≥ÑÏÜç ?¥Ïñ¥Í∞????àÏñ¥??"
    )
...
def create_workshop_route(
    ...
    return _build_state_response(
        ...
        db=db,
        message=f"[{profile.label}] ?¥Îñ§ Î∞©Ïãù?ºÎ°ú ?úÏûë?†Ï? Í≥†Î•¥Î©?Í∑??òÏ???ÎßûÎäî ?®Ï†Ñ???åÌÅ¨???êÎ¶Ñ?ºÎ°ú ?¥Ïñ¥ÏßëÎãà??",
    )
...
def update_quality_level_route(
    ...
    return _build_state_response(
        session=loaded_session,
        db=db,
        message=f"[{profile.label}] ?åÌÅ¨???àÏßà ?àÎ≤®??Î∞òÏòÅ?òÏóà?µÎãà?? ?§Ïùå ?úÏïà???ÑÏû¨ ?àÎ≤® Í∏∞Ï??ºÎ°ú ?§Ïãú Íµ¨ÏÑ±?©Îãà??",
    )
...
def update_visual_approval_route(
    ...
    db.commit()
    db.refresh(artifact)
    return _build_state_response(session=session, db=db, message="?úÍ∞Å???Ä?àÏùò ?πÏù∏ ?ÅÌÉúÍ∞Ä ?ÖÎç∞?¥Ìä∏?òÏóà?µÎãà??")
...
def replace_visual_route(
    ...
    db.commit()
    db.refresh(artifact)
    return _build_state_response(session=session, db=db, message="?àÎ°ú???úÍ∞Å???Ä?àÏùÑ ?ùÏÑ±?àÏäµ?àÎã§.")
...
async def chat_stream_route(
    ...
    if payload.draft_snapshot_markdown and payload.draft_snapshot_markdown.strip():
        ...
        draft_snapshot_context = f"[?†Ï? ÏµúÏã† Ï¥àÏïà ?§ÎÉÖ??\n{snapshot}"
    structured_draft = (
...
def pin_reference_route(
    ...
    return _build_state_response(
        session=loaded_session,
        db=db,
        message="Ï∞∏Í≥†?êÎ£åÎ•?Í≥†Ï†ï?àÏäµ?àÎã§. ?ÑÏû¨ Îß•ÎùΩ ?êÏàò?Ä Ï∞∏Í≥†?êÎ£å ?¨Ïö© Í∞ïÎèÑ??ÎßûÏ∂∞ Î°úÎìúÎßµÏóê Î∞òÏòÅ?©Îãà??",
    )
    ...
    if not requirements["can_render"] and not payload.force:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "?ÑÏû¨ Îß•ÎùΩ ?êÏàò Í∏∞Ï??ºÎ°ú ?ÑÏßÅ Î°úÎìúÎßµÏóê ?ÑÏöî??Îß•ÎùΩ??Î∂ÄÏ°±Ìï©?àÎã§.",
                **requirements,
            },
        )
    ...
    async def generate() -> AsyncIterator[str]:
        ...
        try:
            ...
            if artifact_db is not None and collected:
                ...
                artifact_db.error_message = "?åÎçîÎß??§Ìä∏Î¶ºÏù¥ ?§Ìå®?àÏäµ?àÎã§. ?ÑÏû¨ Ï¥àÏïà Îß•ÎùΩ???êÍ??????§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??"
