...
def render_consultant_diagnosis_pdf(
    *,
    ...
    doc = SimpleDocTemplate(
        ...
        title=str(report_payload.get("title") or "? ë‹ˆ?´ë¦¬ ì§„ë‹¨ ë³´ê³ ??),
        author="? ë‹ˆ?´ë¦¬ ì§„ë‹¨",
    )
    ...
    story.extend(
        [
            Paragraph("? ë‹ˆ?´ë¦¬ ì»¨ì„¤?´íŠ¸ ì§„ë‹¨ ë¦¬í¬??, style_tokens["cover_label"]),
            Paragraph(_escape(str(report_payload.get("title") or "? ë‹ˆ?´ë¦¬ ì§„ë‹¨ ë³´ê³ ??)), style_tokens["cover_title"]),
            Paragraph(_escape(str(report_payload.get("subtitle") or "ê·¼ê±° ì¤‘ì‹¬ ì§„ë‹¨ ê²°ê³¼")), style_tokens["cover_subtitle"]),
            Spacer(1, style_tokens["spacing"]["cover_block_gap"]),
        ]
    )

    cover_meta_rows = [
        ["?€???„ë¡œ?íŠ¸", _escape(str(report_payload.get("student_target_context") or "-"))],
        ["ë¦¬í¬??ëª¨ë“œ", "?„ë¦¬ë¯¸ì—„ 10ìª? if report_mode == "premium_10p" else "ì»´íŒ©???”ì•½"],
        ["?œí”Œë¦?, "?´ë? ?œì? ?œí”Œë¦?],
        [
            "?µì‹¬ ?ì •",
            _escape(str(render_hints.get("one_line_verdict") or "?™ìƒë¶€ ê·¼ê±° ê¸°ë°˜?¼ë¡œ ì§„ë‹¨ ê²°ë¡ ???•ë¦¬?ˆìŠµ?ˆë‹¤.")),
        ],
        [
            "ë¶„ì„ ? ë¢°??,
            f"{int(round(float(render_hints.get('analysis_confidence_score', 0.0)) * 100))}%",
        ],
        ["?ì„± ?œê°", datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M ?¸ê³„?œì???)],
    ]
    ...
    story.append(
        _build_callout(
            text="ì§„ë‹¨?œëŠ” ?™ìƒë¶€ ê·¼ê±°ë¥?ê¸°ë°˜?¼ë¡œ ?‘ì„±?˜ë©°, ë¶ˆí™•?¤í•œ ??ª©?€ ë³„ë„ ê²€ì¦?ë©”ëª¨ë¡??ˆë‚´?©ë‹ˆ??",
            width=doc.width,
    ...
    for group in section_groups:
        ...
        for section_id in available_ids:
            ...
            heading = f"{section_number.get(section_id, 0)}. {str(section.get('title') or 'ì§„ë‹¨ ?¹ì…˜')}"
            ...
            if section_id == "record_baseline_dashboard":
                ...
                if score_groups:
                    for group in score_groups:
                        group_title = str(group.get("title") or "?ìˆ˜ ê·¸ë£¹")
                    ...
                    story.append(Paragraph("?‰ê? ?ìˆ˜", style_tokens["h3"]))
                    story.append(
                        _build_score_table(
            ...
            if section_id == "roadmap" and roadmap:
                story.append(Spacer(1, style_tokens["spacing"]["paragraph_gap"]))
                story.append(Paragraph("?¨ê³„ë³??¤í–‰ ê³„íš", style_tokens["h3"]))
            ...
            if evidence_items and should_render_evidence:
                ...
                for evidence in evidence_items[:max_evidence_cards]:
                    ...
                    source_text = f"{source_label} {page}ìª? if page else source_label
            ...
            if unsupported_claims:
                story.append(
                    _build_callout(
                        text="ê²€ì¦??„ìš”: " + " | ".join(unsupported_claims[:4]),
                        width=doc.width,
                        style=style_tokens["callout"],
            ...
            if verification_needed:
                story.append(Spacer(1, style_tokens["spacing"]["list_item_gap"]))
                story.append(Paragraph("ì¶”ê? ?•ì¸ ?„ìš”", style_tokens["meta_strong"]))
    ...
    if public_appendix_enabled and (uncertainty_notes or appendix_notes):
        story.append(PageBreak())
        story.append(Paragraph("ë¶€ë¡?/ ë¶ˆí™•?¤ì„± ë°?ê²€ì¦?ë©”ëª¨", style_tokens["h2"]))
        if uncertainty_notes:
            story.append(Paragraph("ë¶ˆí™•?¤ì„± ë°?ê²€ì¦?ê²½ê³„", style_tokens["h3"]))
            for note in uncertainty_notes[:max_uncertainty_items]:
                story.append(Paragraph(f"&#8226; {_escape(note)}", style_tokens["bullet"]))
        if appendix_notes:
            story.append(Spacer(1, style_tokens["spacing"]["section_gap"]))
            story.append(Paragraph("?´ì˜/?Œì‹± ë©”ëª¨", style_tokens["h3"]))
            for note in appendix_notes[:max_uncertainty_items]:
                story.append(Paragraph(f"&#8226; {_escape(note)}", style_tokens["bullet"]))

    if public_citations_enabled and citations:
        story.append(PageBreak())
        story.append(Paragraph("ë¶€ë¡?/ ì¶œì²˜ ê·¼ê±° ëª©ë¡", style_tokens["h2"]))
        for citation in citations[:max_citation_items]:
            source = str(citation.get("source_label") or "ì¶œì²˜")
            ...
            prefix = f"{source} ({page_number}ìª?" if page_number else source
            if score is not None:
                prefix = f"{prefix} | ê´€?¨ë„={score} | {support_status}"
    ...
    for idx in range(filler_pages):
        story.append(PageBreak())
        story.append(Paragraph(f"?ˆì§ˆ ?ê? ë©”ëª¨ {idx + 1}", style_tokens["h2"]))
        story.append(
            Paragraph(
                "???˜ì´ì§€??ìµœì¢… ?œì¶œ ??ì§„ë‹¨ ë¬¸ìž¥ê³?ê·¼ê±° ?•í•©?±ì„ ?ê??˜ê¸° ?„í•œ ?•ì¸ ?ì—­?…ë‹ˆ?? "
                "ë¶ˆí™•?¤í•œ ??ª©?€ ë°˜ë“œ??'ì¶”ê? ?•ì¸ ?„ìš”'ë¡??œì‹œ??ì£¼ì„¸??",
                style_tokens["body"],
            )
        )
        story.append(Paragraph("&#8226; ?µì‹¬ ì£¼ìž¥ë§ˆë‹¤ ì¶œì²˜ ?¼ì¸ 1ê°??´ìƒ ?°ê²°", style_tokens["bullet"]))
        story.append(Paragraph("&#8226; ê²€ì¦?ë¯¸ì™„ë£?ë¬¸ìž¥?€ ì¶”ê? ?•ì¸ ?„ìš”ë¡??œì‹œ", style_tokens["bullet"]))
        story.append(Paragraph("&#8226; ê³¼ìž¥Â·?ˆìœ„Â·ê·¼ê±° ë¶€ì¡??œí˜„ ?œê±°", style_tokens["bullet"]))
...
def _render_section_body(
    ...
    *,
    ...
    if not lines:
        return [Paragraph("?´ìš©???„ì§ ì¤€ë¹„ë˜ì§€ ?Šì•˜?µë‹ˆ??", body_style)]
    line_caps = {
    ...
    if len(lines) > max_lines:
        lines = [*lines[:max_lines], "- ë³¸ë¬¸ ë¶„ëŸ‰?€ ?˜ì´ì§€ ê°€?…ì„±???„í•´ ?”ì•½?ˆìŠµ?ˆë‹¤."]
    rendered: list[Any] = []
    for line in lines:
        stripped = line.strip()
        if not stripped:
...
def _draw_page_chrome(
    ...
    canvas.drawString(doc.leftMargin, height - 11, "? ë‹ˆ?´ë¦¬ ì§„ë‹¨ ë³´ê³ ??)
    ...
    canvas.drawString(doc.leftMargin, 18, "ê·¼ê±° ì¤‘ì‹¬ ì§„ë‹¨ ë¦¬í¬??)
    canvas.drawRightString(width - doc.rightMargin, 18, f"{canvas.getPageNumber()} ?˜ì´ì§€")
    canvas.restoreState()
...
def _support_status_label(value: str) -> str:
    ...
    mapping = {
        ...
        "partial": "ë¶€ë¶?ê²€ì¦?,
        "needs_review": "ê²€???„ìš”",
        "needs_verification": "ê²€ì¦??„ìš”",
        "unsupported": "ê·¼ê±° ë¶€ì¡?,
    }
    return mapping.get(normalized, "?•ì¸ ?„ìš”")
