...
try:
    from unifoli_parsers.opendataloader_adapter import OpenDataLoaderAdapter, OpenDataLoaderError
except ImportError:
...
SECTION_PATTERNS: dict[str, tuple[str, ...]] = {
    "ÍµêÍ≥º?ôÏäµÎ∞úÎã¨?ÅÌô©": ("ÍµêÍ≥º?ôÏäµÎ∞úÎã¨?ÅÌô©", "ÍµêÍ≥º ?ôÏäµ Î∞úÎã¨ ?ÅÌô©", "?∏Î??•Î†• Î∞??πÍ∏∞?¨Ìï≠", "?∏Ìäπ"),
    "Ï∞ΩÏùò?ÅÏ≤¥?òÌôú??: ("Ï∞ΩÏùò?ÅÏ≤¥?òÌôú??, "?êÏú®?úÎèô", "?ôÏïÑÎ¶¨Ìôú??, "ÏßÑÎ°ú?úÎèô", "Î¥âÏÇ¨?úÎèô"),
    "?âÎèô?πÏÑ± Î∞?Ï¢ÖÌï©?òÍ≤¨": ("?âÎèô?πÏÑ± Î∞?Ï¢ÖÌï©?òÍ≤¨", "?âÎèô?πÏÑ±", "Ï¢ÖÌï©?òÍ≤¨"),
    "?ÖÏÑú?úÎèô": ("?ÖÏÑú?úÎèô", "?ÖÏÑú ?úÎèô"),
    "?òÏÉÅÍ≤ΩÎ†•": ("?òÏÉÅÍ≤ΩÎ†•", "?òÏÉÅ Í≤ΩÎ†•"),
}

HEADER_ALIASES: dict[str, tuple[str, ...]] = {
    "school_year": ("?ôÎÖÑ", "school_year", "school year"),
    "semester": ("?ôÍ∏∞", "semester"),
    "subject_group": ("ÍµêÍ≥º", "ÍµêÍ≥ºÍµ?, "?ÅÏó≠", "Í≥ºÎ™©Íµ?, "subject_group"),
    "subject_name": ("Í≥ºÎ™©", "ÍµêÍ≥ºÎ™?, "subject", "subject_name"),
    "unit_or_credit": ("?®ÏúÑ", "?®ÏúÑ??, "?¥Ïàò", "credit", "unit"),
    "achievement": ("?±Ï∑®??, "?±Í∏â", "?êÏ†ê??, "achievement"),
    "special_notes_text": (
        "?∏Î??•Î†• Î∞??πÍ∏∞?¨Ìï≠",
        "?∏Ìäπ",
        "?πÍ∏∞?¨Ìï≠",
...
NEIS_SECTION_TOKENS: tuple[str, ...] = (
    ...
    "Ï∂úÍ≤∞?ÅÌô©",
    "?òÏÉÅÍ≤ΩÎ†•",
    "?êÍ≤©Ï¶?Î∞??∏Ï¶ù Ï∑®Îìù?ÅÌô©",
    "Ï∞ΩÏùò??Ï≤¥Ìóò?úÎèô",
    "ÍµêÍ≥º?ôÏäµÎ∞úÎã¨?ÅÌô©",
    "?âÎèô?πÏÑ± Î∞?Ï¢ÖÌï©?òÍ≤¨",
    "?ÖÏÑú?úÎèô",
)
    ...
    with pdfplumber.open(file_path) as pdf:
        ...
        for page in pdf.pages:
            ...
            looks_like_table = line_count >= 14 or any(
                token in text for token in ("?ôÎÖÑ", "?ôÍ∏∞", "?∏Î??•Î†•", "Í≥ºÎ™©", "Ï∞ΩÏùò?ÅÏ≤¥?òÌôú??)
            )
...
def _provider_result_to_normalized_artifact(
    ...
    *,
    ...
    return {
        "schema_version": "unifoli.neis.normalized.v1",
...
def map_neis_semantics(
    ...
    *,
    ...
    for chain_index, chain in enumerate(stitched_artifact.get("table_chains", [])):
        ...
        if section_type == "ÍµêÍ≥º?ôÏäµÎ∞úÎã¨?ÅÌô©":
    ...
    return {
        "schema_version": "unifoli.neis.document.v1",
...
def parse_pdf_with_neis_pipeline(
    ...
    *,
    ...
    return ParsedDocumentPayload(
        ...
        raw_artifact=raw_artifact,
        masked_artifact={
            "schema_version": "unifoli.neis.masked_artifact.v1",
            "content_text": content_text,
            "content_markdown": content_markdown,
            "chunk_evidence_map": chunk_evidence_map,
        },
...
def _map_activity_records(
    ...
    for row_index, row in enumerate(data_rows):
        ...
        bbox_refs = [cell.get("bbox") for cell in row.get("cells", []) if cell.get("bbox") is not None]
        confidence = round(min(0.9, 0.6 + (0.1 if len(text) >= 20 else 0.0) + (0.1 if section_type != "ÎØ∏Î∂ÑÎ•? else 0.0)), 2)
...
def _map_text_only_sections(
    ...
    for page_number, text in sorted(_page_text_lookup(stitched_artifact).items()):
        ...
        masking_result = masking_pipeline.mask_text(text)
        record_confidence = 0.62 if section_type != "ÎØ∏Î∂ÑÎ•? else 0.48
