...
def _build_minimal_result_payload() -> DiagnosisResultPayload:
    return DiagnosisResultPayload.model_validate(
        {
            "headline": "ê¸°ì´ˆ ì§„ë‹¨ ?¤ë“œ?¼ì¸",
            "strengths": ["?êµ¬ ì£¼ì œ???¼ê??±ì´ ë³´ìž…?ˆë‹¤."],
            "gaps": ["ê·¼ê±° ë¬¸ìž¥ê³??¸ìš© ?°ê²°????ëª…í™•???´ì•¼ ?©ë‹ˆ??"],
            "recommended_focus": "ê·¼ê±°-ì£¼ìž¥ ë§¤í•‘ ê°•í™”",
            "risk_level": "warning",
            "next_actions": ["?µì‹¬ ì£¼ìž¥ 3ê°œì— ê·¼ê±° ì¶œì²˜ë¥??°ê²°?˜ì„¸??"],
            "recommended_topics": ["?„ê³µ ?°ê³„ ?êµ¬ ?¬í™”"],
            "citations": [
                {
                    "source_label": "?™ìƒë¶€ ê¸°ë¡",
                    "page_number": 2,
                    "excerpt": "?êµ¬ ?œë™??ê³¼ì •ê³?ê²°ê³¼ê°€ ?•ë¦¬??,
                    "relevance_score": 1.7,
                }
            ],
        }
...
def test_build_consultant_report_payload_contains_expected_sections(monkeypatch) -> None:
    async def fake_narratives(**kwargs):  # noqa: ANN003
        return report_service._ConsultantNarrativePayload(
            executive_summary="?”ì•½ ë¬¸ìž¥",
            final_consultant_memo="ìµœì¢… ë©”ëª¨ ë¬¸ìž¥",
        )
        ...
        SimpleNamespace(
            parse_metadata={
                "student_record_structure": {
                    "section_density": {"?¸íŠ¹": 0.8, "ì°½ì²´": 0.4},
                    "weak_sections": ["ì§„ë¡œ"],
                    "timeline_signals": ["2?™ë…„", "3?™ë…„"],
                    "activity_clusters": ["?êµ¬/?¤í—˜"],
                    "subject_major_alignment_signals": ["?„ê³µ ?°ê³„ ë¬¸ìž¥ ?•ì¸"],
                    "continuity_signals": ["?„ì† ?êµ¬ ê³„íš"],
                    "process_reflection_signals": ["?œê³„?€ ê°œì„ ??],
                    "uncertain_items": [],
                }
            }
        )
...
def test_generate_consultant_report_artifact_fallbacks_to_failed_status(monkeypatch) -> None:
    ...
    project = SimpleNamespace(
        id="project-3",
        title="?¤íŒ¨ ?´ë°± ?„ë¡œ?íŠ¸",
        target_university="?°ì„¸?€?™êµ",
        target_major="?„ê¸°?„ìžê³µí•™",
    )
...
def test_generate_narratives_uses_deterministic_fallback_when_prompt_registry_missing(monkeypatch) -> None:
    ...
    payload = asyncio.run(
        report_service._generate_narratives(  # noqa: SLF001
            ...
            result=_build_minimal_result_payload(),
            document_structure={"weak_sections": ["ì§„ë¡œ"], "section_density": {"?¸íŠ¹": 0.5}},
            uncertainty_notes=["?™ìƒë¶€ ?¼ë? ?˜ì´ì§€???ë… ? ë¢°?„ê? ??Šµ?ˆë‹¤."],
        )
    )
        ...
        bank.append(
            {
                ...
                "quote": f"ì§€?ê???ê±´ì¶• ?êµ¬ ê·¼ê±° ë¬¸ìž¥ {index}",
                "major_relevance": ["ê±´ì¶•", "?˜ê²½"],
                "process_elements": {
                    "motivation": True,
                    "method": True,
                    "finding": True,
...
def test_score_groups_keep_student_and_system_scores_separate(monkeypatch) -> None:
    async def fake_narratives(**kwargs):  # noqa: ANN003
        return report_service._ConsultantNarrativePayload(
            executive_summary="?”ì•½",
            final_consultant_memo="ë©”ëª¨",
        )

    monkeypatch.setattr(report_service, "_generate_narratives", fake_narratives)

    run = SimpleNamespace(id="run-score-groups")
    project = SimpleNamespace(id="project-score-groups", title="?ŒìŠ¤??, target_university="?œìš¸?€?™êµ", target_major="ê±´ì¶•?™ê³¼")
    result = _build_minimal_result_payload()
    documents = [
        SimpleNamespace(
            parse_metadata={
                ...
                "student_record_structure": {
                    "section_density": {"êµê³¼?™ìŠµë°œë‹¬?í™©": 0.9, "ì°½ì²´": 0.8, "?‰ë™?¹ì„±": 0.7},
...
def test_contradiction_check_blocks_premium_render(monkeypatch) -> None:
    ...
    project = SimpleNamespace(id="project-contradiction", title="?ŒìŠ¤??, target_university="?œìš¸?€?™êµ", target_major="ê±´ì¶•?™ê³¼")
    result = _build_minimal_result_payload()
    documents = [
        SimpleNamespace(
            parse_metadata={
                "student_record_structure": {
                    "section_density": {"êµê³¼?™ìŠµë°œë‹¬?í™©": 1.0},
                    "weak_sections": ["êµê³¼?™ìŠµë°œë‹¬?í™©"],
                    "coverage_check": {"coverage_score": 1.0, "reanalysis_required": False, "missing_required_sections": []},
                    "contradiction_check": {
                        "passed": False,
                        "items": [{"section": "êµê³¼?™ìŠµë°œë‹¬?í™©", "reason": "weak_or_missing_conflicts_with_density"}],
                    },
                }
            }
        )
...
def test_premium_report_sections_keep_diverse_evidence_anchors(monkeypatch) -> None:
    ...
    project = SimpleNamespace(id="project-diverse-anchors", title="?ŒìŠ¤??, target_university="?œìš¸?€?™êµ", target_major="ê±´ì¶•?™ê³¼")
    result = _build_minimal_result_payload()
    documents = [
        SimpleNamespace(
            parse_metadata={
                ...
                "student_record_structure": {
                    "section_density": {"êµê³¼?™ìŠµë°œë‹¬?í™©": 0.95, "?¸íŠ¹": 0.95, "ì°½ì²´": 0.9, "?‰ë™?¹ì„±": 0.8},
                    "coverage_check": {"coverage_score": 1.0, "reanalysis_required": False, "missing_required_sections": []},
                    "contradiction_check": {"passed": True, "items": []},
                },
            }
