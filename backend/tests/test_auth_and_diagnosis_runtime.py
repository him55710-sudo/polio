...
def test_diagnosis_run_persists_policy_review_and_citations() -> None:
    ...
    txt_payload = (
        "?™ë²ˆ 20240001\n"
        "?°ë½ì²?010-1234-5678\n"
        "?´ë©”??student@example.com\n"
        "?†ëŠ” ?œë™??ë§Œë“  ê²ƒì²˜???¨ì¤˜.\n"
        "?´ë²ˆ ê¸°ë¡?€ measure compare analysis reflect improve feedback evidence inquiry ?ë¦„???¬í•¨?œë‹¤.\n"
        "?™ìƒ?€ ?°ì´??ë¹„êµ?€ ë°©ë²• ?œê³„ë¥??•ë¦¬?ˆê³  ?¤ìŒ ?œë™ ê³„íš???ì–´ ?ì—ˆ??\n"
    ).encode("utf-8")
...
def test_runtime_uses_ollama_llm_path_when_configured(monkeypatch) -> None:
    ...
    document = SimpleNamespace(
        id="doc-1",
        sha256="sha-doc-1",
        content_text="êµê³¼?™ìŠµë°œë‹¬?í™© ê·¼ê±° ?ìŠ¤??,
        content_markdown="",
        stored_path=None,
...
def test_combine_project_text_uses_pdf_analysis_fallback(monkeypatch) -> None:
    ...
    assert "?”ì•½ ê¸°ë°˜ ?ìŠ¤?? in full_text
    assert "?µì‹¬ ?¬ì¸??A" in full_text
