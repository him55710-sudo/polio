...
FIXED_GREETING = "?ˆë…•?˜ì„¸?? ?´ë–¤ ì£¼ì œ??ë³´ê³ ?œë? ?¨ë³¼ê¹Œìš”?"
...
class _FakeGuidedChatLLM:
    ...
    async def generate_json(self, prompt, response_model, system_instruction=None, temperature=0.2):  # noqa: ANN001
        suggestions = [
            TopicSuggestion(
                id=f"topic-{index + 1}",
                title=f"?ŒìŠ¤??ì£¼ì œ {index + 1}",
                why_fit_student="?•ì¸??ë§¥ë½ ë²”ìœ„ ?ˆì—???ˆì „?˜ê²Œ ì§„í–‰ ê°€?¥í•œ ì£¼ì œ?…ë‹ˆ??",
                link_to_record_flow="ê¸°ë¡ ?ë¦„ê³??°ê²° ê°€?¥í•œ ë²”ìœ„?ì„œ ?œì•ˆ?œë¦½?ˆë‹¤.",
                link_to_target_major_or_university=None,
                novelty_point="ê¸°ì¡´ ?ë¦„??ë³´ìˆ˜?ìœ¼ë¡??•ì¥?©ë‹ˆ??",
                caution_note=None,
            )
...
def _create_project(client: TestClient, headers: dict[str, str]) -> str:
    response = client.post(
        "/api/v1/projects",
        headers=headers,
        json={"title": "Guided Chat Test Project", "target_major": "?˜í•™"},
    )
...
def test_topic_suggestions_always_return_exactly_three_items(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            headers=headers,
            json={"project_id": project_id, "subject": "?˜í•™"},
        )
...
def test_missing_diagnosis_data_returns_limited_context_note(monkeypatch) -> None:
    ...
    assert note
    assert ("?œí•œ" in note) or ("ë¶€ì¡? in note)
    assert len(payload["suggestions"]) == 3
...
def test_topic_selection_returns_richer_starter_draft(monkeypatch) -> None:
    ...
    assert "## ì¦ê±°-?ˆì „ ?‘ì„± ê²½ê³„" in starter
    assert "## Evidence Memo" in starter
    assert "## ?„ì… ë¬¸ë‹¨(ì´ˆì•ˆ)" in starter
