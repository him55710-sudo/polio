# UniFoli Backend CTO Assessment

> CTO 시각에서 정리한 UniFoli admissions intelligence backend 상태 문서
> 기준일: 2026-03-21
> 평가 기준: 현재 레포지토리, 구현된 코드, 로컬 검증 결과

이 문서는 현재 구성된 UniFoli 백엔드가 어떤 구조를 가지고 있는지, 무엇이 이미 구현되어 있는지, 어떤 오픈소스를 쓰고 있는지, 어디까지가 자동화 가능하고 어디부터는 사람이 직접 해야 하는지, 그리고 현재 상태를 얼마나 냉정하게 평가해야 하는지를 정리한 운영용 문서다.
...
현재 UniFoli 백엔드는 단순한 챗봇 골격이 아니라, 공식 입시 자료를 수집하고 문서를 파싱하고 chunk와 claim을 저장하고 학생 업로드 파일을 분석 가능한 단위로 전환하는 admissions-aware backend foundation으로 설계되어 있다.
...
UniFoli 백엔드의 핵심 철학은 분명하다. 이 시스템은 학생에게 허위 기록을 만들어주거나 보기 좋은 문장을 대신 꾸며주는 도구가 아니라, 공식 평가 기준이 실제 학생 활동과 학생부 기록을 어떻게 해석하는지를 근거 중심으로 설명하는 admissions intelligence system이 되려 한다.
...
이 점은 UniFoli가 흔한 AI 포트폴리오 생성기와 갈라지는 지점이다.
...
PolicyFlag, ReviewTask, ResponseTrace, Citation이 데이터 모델과 서비스 계층에 들어와 있다. 이것은 UniFoli가 제품 철학을 backend에 반영하고 있다는 강력한 신호다.
...
이 흐름은 현재 UniFoli 백엔드의 가장 큰 장점이다. 많은 AI 서비스는 문서를 넣고 결과만 뱉는다. 그런데 UniFoli는 intermediate object를 거의 다 저장한다. 이것은 미래 확장성과 운영 신뢰성을 크게 높인다.
...
이것은 중요하다. 입시 제품에서 "아키텍처 예쁘게 설계함"과 "실제로 ingestion slice가 돈다"는 완전히 다르다. UniFoli는 이제 후자에 진입했다.
...
현재 UniFoli 백엔드의 가장 큰 강점 중 하나는 데이터 모델이다. 일반적인 AI 앱은 request와 response만 저장하고 끝나지만, UniFoli는 입시 제품에 필요한 운영 객체를 상당히 잘 쪼개어 모델링했다.
...
이 구조 덕분에 UniFoli는 단순히 "답이 맞는가"만 보는 게 아니라 "어떤 공식 자료의 어느 부분을 보고 이런 해석이 나왔는가"까지 보여줄 수 있다.
...
현재 UniFoli 백엔드는 오픈소스 선택이 비교적 합리적이다. 무엇보다 "한 명 또는 소수 팀이 운영 가능한 범위"를 크게 벗어나지 않는다.
...
UniFoli의 경쟁력은 AI 자동화에만 있지 않다. 오히려 이 제품은 사람이 어디에 개입해야 하는지를 정확히 아는 것이 중요하다.
...
> UniFoli는 human-in-the-loop를 product cost가 아니라 product moat로 봐야 한다.
...
즉, 지금 UniFoli의 핵심 병목은 모델이 아니라 데이터다.
...
이 비교는 특정 회사를 지목한 시장 분석이 아니라, 흔한 제품 접근 방식과 UniFoli의 구조를 비교한 것이다.
...
> UniFoli의 진짜 차별점은 "더 화려한 생성"이 아니라
...
이 순서를 지키면 UniFoli는 단순한 AI 작성기가 아니라, 한국 입시형 고신뢰 백엔드로 발전할 수 있다.
